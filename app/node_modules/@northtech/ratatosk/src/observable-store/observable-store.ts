...
export class ObservableStore<StoreDataT> extends Observable<StoreDataT> {
  ...
  constructor(initialValue: StoreDataT,
    ...
    if (fieldsToPersist) {
      ...
      (Object.keys(this.#deserializers) as (keyof StoreDataT)[]).forEach(key => {
        try {
          const storedJson = localStorage.getItem(this.#localStoragePrefix + key.toString());
          if (storedJson != null) {
  ...
  private persist<KeyInStoreT extends keyof StoreDataT>(key: KeyInStoreT, value: StoreDataT[KeyInStoreT]): void {
    ...
    if (serializer) {
      try {
        ...
        if (json == null) {
          localStorage.removeItem(this.#localStoragePrefix + key.toString());
        } else {
          localStorage.setItem(this.#localStoragePrefix + key.toString(), json);
        }
      } catch (e) {
  ...
  */
   ...
   * Be aware that this can only clone data. Object prototypes and methods will not be cloned.
   */
  ...
  observeClone<KeyInStoreT extends keyof StoreDataT>(keyOrKeys: KeyInStoreT | KeyInStoreT[], ...additionalKeys: KeyInStoreT[]): Observable<StoreDataT[KeyInStoreT]> | Observable<Pick<StoreDataT, KeyInStoreT>> {
    ...
    if (Array.isArray(input)) {
      return this.observeObject(...input).pipe(
        map(o => structuredClone(o))
      );
    } else {
      return this.observeValue(input).pipe(
        map(o => structuredClone(o))
      );
    }
  }
  ...
  */
   ...
   */
  synchronizeWith<OtherStoreDataT, KeyT extends keyof (StoreDataT | OtherStoreDataT)>(
    otherStore: ObservableStore<OtherStoreDataT>,
    ...keys: KeyT[]): void {
    const data = otherStore.getValue() as any;
    keys.forEach(k => {
      this.set(k, data[k]);
      otherStore.observe(k).subscribe(v => this.set(k, v as any));
      this.observe(k).subscribe(v => otherStore.set(k, v as any));
    });
  }
}
