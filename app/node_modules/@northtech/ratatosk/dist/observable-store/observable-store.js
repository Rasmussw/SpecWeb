...
import { map, Observable, pairwise, ReplaySubject, share, startWith, takeUntil } from 'rxjs';
...
export class ObservableStore extends Observable {
     ...
     * consists of keys matching the keys in `StoreDataT´, with either a boolean value (true if the value should be
    ...
    constructor(initialValue, fieldsToPersist, storeNamespace) {
        ...
        if (fieldsToPersist) {
            ...
            Object.keys(__classPrivateFieldGet(this, _ObservableStore_deserializers, "f")).forEach(key => {
                var _a, _b;
                try {
                    const storedJson = localStorage.getItem(__classPrivateFieldGet(this, _ObservableStore_localStoragePrefix, "f") + key.toString());
                    if (storedJson != null) {
                    ...
                    console.log('Error when restoring ' + key.toString() + ' from localStorage', e);
                }
            });
        }
    }
    ...
    persist(key, value) {
        ...
        if (serializer) {
            try {
                ...
                if (json == null) {
                    localStorage.removeItem(__classPrivateFieldGet(this, _ObservableStore_localStoragePrefix, "f") + key.toString());
                }
                else {
                    localStorage.setItem(__classPrivateFieldGet(this, _ObservableStore_localStoragePrefix, "f") + key.toString(), json);
                }
            }
            catch (e) {
                console.log('error when storing ' + key.toString(), value, e);
            }
        }
    }
    ...
    observeValue(key) {
        return this.pipe(map(data => data[key]), distinctUntilChanged((a, b) => deepEquals(a, b)), share({ connector: () => new ReplaySubject(1) }));
    }
    observeObject(...keys) {
        return this.pipe(map(data => {
            ...
            return o;
        }), distinctUntilChanged((a, b) => deepEquals(a, b)), share({ connector: () => new ReplaySubject(1) }));
    }
    observePairwise(keyOrKeys, ...additionalKeys) {
        ...
        return (Array.isArray(input) ? this.observeObject(...input) : this.observeObject(input)).pipe(startWith(undefined), pairwise(), share({ connector: () => new ReplaySubject(1) }));
    }
    ...
    */
    ...
    /**
     * Transfers the given fields from the other store to this, and keeps the two stores in sync after that.
     * Setting a value in one will cause it to also be set in the other, and vice versa.
     * This is particularly useful for sharing data from a minimal, application-wide state to lazy-loaded daughter modules.
     * @param otherStore The store to synchronize data with. The given store will function as master for the initial
     * synchronization, overriding values in this store.
     * @param keys The fields to synchronize. They must exist in both stores, and have compatible data types.
     */
    synchronizeWith(otherStore, ...keys) {
        const data = otherStore.getValue();
        keys.forEach(k => {
            this.set(k, data[k]);
            otherStore.observe(k).subscribe(v => this.set(k, v));
            this.observe(k).subscribe(v => otherStore.set(k, v));
        });
    }
}
